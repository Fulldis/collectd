...
int ci_get_string(const oconfig_value_t *val, char **string) {
  ...
  const char *field = val->value.string;
  *string = calloc(strlen(field) + 1, sizeof(char));
  strcpy(*string, field);
  return 0;
}
...
int ci_get_input(const oconfig_item_t *input, char **input_name, int64_t *input_shape) {
  if (input->children_num != 2) {
    ERROR("error parsing input config: input config has fields, exactly 2 fields required, Name and Size");
    return 1;
  }
...
int ci_get_output(const oconfig_item_t *output, char **output_name) {
  if (output->children_num != 1) {
    ERROR("error parsing output config: output has %d fields, exactly 1 field required: `Name`", output->children_num);
    return 1;
  }
...
int ci_get_inputs(const oconfig_item_t *ci, PluginConfig *cfg) {
...
int ci_get_outputs(const oconfig_item_t *ci, PluginConfig *cfg) {
  size_t num = ci->children_num;
