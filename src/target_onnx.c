...
bool prepare_inputs(const metric_family_t *fam, plugin_config_t *cfg, float **inputs) {
  ...
  for (size_t i = 0; i < cfg->inputs_len; i++) {
    ...
    int err = uc_get_history_by_name(cfg->input_names[i], tmp, cfg->input_shapes[i]);
    if (err == ENOENT) {
...
static int tt_invoke(metric_family_t const *fam, notification_meta_t **, void **user_data) {
